...
function getTitle(favorite) {
  let title = ''
  const serviceName = favorite?.service_at_location?.service.name ?? favorite.service.name
  const locationName = favorite?.service_at_location?.location.name ?? favorite.service.organization.name
